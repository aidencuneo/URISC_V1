...
class Variable:
    ...
    def get_initial_value(self):
        ...
        for bit_index in range(self.bit_size):
            ...
            # If this bit is 1, add a line to the code which inverts this bit
            if bit:
                out += bit_name + ' ?\n'
    ...
    def set_value(self, new_bits):
        ...
        for bit_index in range(self.bit_size):
            bit_name = self.names[bit_index]
            bit = self.bits[bit_index]
            # New bits will usually be given as '0' or '1'
            new_bit = int(new_bits[bit_index])

            # If the xor of the old and new bits is 1, then invert the old bit
            # (If the old and new bits are different)
            if bit ^ new_bit:
                out.append(bit_name + ' ?\n')

            # Set old bits to new bits
...
class URISC_V1_Extended:
    ...
    def __init__(self, code):
        ...
        for num in range(256):
            ...
            self.var[str(num)] = Variable('0', [int(i) for i in format(num, '08b')])
    ...
    def compln(self, line):
        ...
        if line[0] == 'invert':
            ...
            if name not in self.protected_vars and name in self.var:
                # If this is already a variable, change it
                print('YUO!', self.var[name].bits, exp.bits)
                out = self.var[name].set_value(exp.bits)
                print(out, self.var[name].bits)
            else:
                # If this isn't a variable yet, create it
                self.var[name] = Variable(name, exp.bits)
                out = self.var[name].get_initial_value()

            var = self.var[name]
            print(var.name, var.bits, var.bit_size)
            ...
            #     # Invert name if expression result
            #     exp + ' ' + label_2 + '\n' +
            #     name + ' ?\n' +
            #     label_2 + '\n' +
            #     exp + ' ?\n'
